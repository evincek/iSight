...
-- with "function category_key(text) does not exist", which is the correct
-- outcome: the index cannot be built on an unmerged table anyway.
--
...
create unique index if not exists categories_user_norm_name_key
  on categories (user_id, category_key(name));
